(ns ^{:doc "Render all the main pages of a very simple Wiki engine."
 ...
 smeagollib.routes.wiki
  (:require [cemerick.url :refer (url-encode url-decode)]
            ...
            [smeagollib.diff2html :as d2h]
            [smeagollib.finder :refer [find-image]]
            [smeagollib.formatting :refer [md->html]]
            [smeagollib.history :as hist]
            [smeagollib.include.resolve-local-file :as resolve]
            [smeagollib.include :as include]
            [smeagollib.layout :as layout]
            [smeagollib.local-links :refer [local-links]]
            [smeagollib.routes.admin :as admin]
            [smeagollib.sanity :refer [show-sanity-check-error]]
            [smeagollib.uploads :as ul]
            [smeagollib.util :refer [content-dir get-message
                                     get-servlet-context-path local-url
                                     local-url-base standard-params
    ...
    (let [params (keywordize-keys (:params request))
      ...
      (log/info
       (format (if-not
               ...
               user file-path))
      (if src-text (process-source params suffix request)
